...
pub fn parse_memento_from_str(message: &String, game_data: &Mutex<GameData>) {
    let mut game_data: MutexGuard<GameData> = game_data.lock().unwrap();
    ...
    loop {
        match reader.read_event(&mut buf) {
            Ok(Event::Start(ref e)) => {
                match e.name() {
                    b"state" => {
                        let memento_turn = String::from_utf8(e.try_get_attribute("turn").unwrap().unwrap().value.to_vec()).unwrap().parse::<i8>().unwrap();
                        game_data.turn = memento_turn;
                    },
                    b"list" => {
                        if !first_y {
                            y += 1;
